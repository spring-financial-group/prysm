...
func TestProcessAttesterSlashings_DataNotSlashable(t *testing.T) {
	...
	_, err = blocks.ProcessAttesterSlashings(context.Background(), beaconState, ss, v.SlashValidator, maxExitEpoch, churn)
...
func TestProcessAttesterSlashings_AppliesCorrectStatus(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			newState, err := blocks.ProcessAttesterSlashings(context.Background(), tc.st, []ethpb.AttSlashing{tc.slashing}, v.SlashValidator, maxExitEpoch, churn)
			require.NoError(t, err)
