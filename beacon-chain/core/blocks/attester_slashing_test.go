...
func TestProcessAttesterSlashings_DataNotSlashable(t *testing.T) {
	...
	_, err = blocks.ProcessAttesterSlashings(context.Background(), beaconState, ss, v.SlashValidator, v.MaxExitEpochAndChurn(beaconState))
...
func TestProcessAttesterSlashings_AppliesCorrectStatus(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			newState, err := blocks.ProcessAttesterSlashings(context.Background(), tc.st, []ethpb.AttSlashing{tc.slashing}, v.SlashValidator, v.MaxExitEpochAndChurn(tc.st))
			require.NoError(t, err)
